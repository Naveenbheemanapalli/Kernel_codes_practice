/*Brief Description:
 *
 * This Simple kernel module to demo kernel panic in the Linux Kernel
 *
...
#define MODULE_NAME "sysfs_example"			//defining the name of the kernel module
...
static ssize_t training_store(struct kobject *kobj,struct kobj_attribute *attr,const char *buf,size_t count){
	...
	if(strncmp(buf,"panic",5)==0){
		pr_info("panic arriving...\n");
		panic("paniced...\n"); //panicing/creating kernel oops with the built in panic function method
		//*(int *)0 = 0; 						//creating a buggy kernel panic with the NULL pointer assignment method
	}
	return count;
}
